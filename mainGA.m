...
for i = 1:nPop
    ...
    % check constraint
    % pop(i).isDAG = IsDAG(pop(i).Position, model);
    % disp(['isDaG ' num2str(pop(i).isDAG)]);
...
pop = pop(SortOrder);
% isdags = [pop.isDAG];
% [isdags, SortOrder] = sort(isdags, 'descend');
% pop = pop(SortOrder);
...
for it = 1:MaxIt
    ...
    for k = 1:nc/2
        ...
        % popc(k, 1).isDAG = IsDAG(popc(k, 1).Position,model);
        % popc(k, 2).isDAG = IsDAG(popc(k, 2).Position,model);
    ...
    for k = 1:nm
        ...
        % popm(k).isDAG = IsDAG(popm(k).Position,model);
